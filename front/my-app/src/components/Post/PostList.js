...
const PostList = ({ postUrl, auth, user, useModal }) => {
  const [posts, setPosts] = useRemoteData(postUrl, auth);
  const postCount = posts.length;
  return (
    <section className="main-section">
      <p className="unfocus-text medium-text ">
      ...
      {posts[0] ? (
        ...
        <DivHolder className="div-holder error">
          ...
          <p className="text-center">
            Parece que <Link to={`/u/${user.userName}`}>u/{user.userName}</Link>{' '}
            no ha posteado nada aún
          </p>
