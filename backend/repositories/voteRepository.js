...
async function addVote(userId, threadId, voteType) {
  ...
  const selectQuery =
    'SELECT * FROM user_thread_vote WHERE userId = ? AND threadId = ?';
  ...
  if (vote) {
    const updateQuery =
      'UPDATE user_thread_vote SET voteType = ? WHERE userId = ? AND threadId = ?';
    await pool.query(updateQuery, [voteType, userId, threadId]);
  }
  // If vote dont exist create de vote
  else if (!vote[0]) {
    const insertQuery =
      'INSERT INTO user_thread_vote (userId, threadId, voteType, voteDate) VALUES (?,?,?,?)';
    await pool.query(insertQuery, [userId, threadId, voteType, time]);
  }

  const [newVote] = await await pool.query(selectQuery, [userId, threadId]);
